const { ApplicationCommandType, EmbedBuilder, ActionRowBuilder, ButtonStyle, ButtonBuilder, RoleSelectMenuBuilder, ChannelSelectMenuBuilder, CategoryChannel, ChannelType, ModalBuilder, TextInputBuilder, StringSelectMenuBuilder, AttachmentBuilder, UserSelectMenuBuilder, MessageAttachment } = require("discord.js");
    ...
    run: async (interaction, client) => {
        ...
        if (customId == "deletar_ticket") {
            ...
            if (interaction.channel && interaction.channel.isTextBased()) {
                ...
                const logs = interaction.client.channels.cache.get(definition.channels.logs);
                if (!logs) {
                    console.error("O canal de logs não foi encontrado.");
                    return;
                }

                const transcriptMessage = await logs.send({
                    embeds: [
                        new EmbedBuilder()
                ...
                const firstAttachmentUrl = transcriptMessage.attachments.first()?.url;
                if (firstAttachmentUrl) {
                    const buttontranscript = new ActionRowBuilder()
                        .addComponents(
                            new ButtonBuilder()
                                .setEmoji("🌐")
                                .setLabel("Ir ao transcript")
                                .setStyle(ButtonStyle.Link)
                                .setURL(`https://mahto.id/chat-exporter?url=${firstAttachmentUrl}`),
                            new ButtonBuilder()
                                .setEmoji("🌐")
                                .setLabel("Baixar Transcript")
                                .setStyle(ButtonStyle.Link)
                                .setURL(`${firstAttachmentUrl}`),
                        );

                    await logs.send({ components: [buttontranscript] });
